...
public class PrincipalBearingPrincipalResolver extends PersonDirectoryPrincipalResolver {
    ...
    @Override
    public Principal resolve(Credential credential, Principal currentPrincipal, AuthenticationHandler handler) {
        Principal newPrincipal =  super.resolve(credential, currentPrincipal, handler);
        Map attributes = newPrincipal.getAttributes();
        if (currentPrincipal.getAttributes() != null && !currentPrincipal.getAttributes().isEmpty()) {
            attributes.putAll(currentPrincipal.getAttributes());
        }
        return principalFactory.createPrincipal(newPrincipal.getId(), attributes);
    }
